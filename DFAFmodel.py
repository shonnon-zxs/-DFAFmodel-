...
class Net(nn.Module):
    ...
    def forward(self, v, b, q, v_mask, q_mask):
        ...
        v = v.transpose(1,2).contiguous()  # [batch, num_obj, 2048]
        b = b.transpose(1,2).contiguous()  # 没用到 transpose(1,2) = transpose(2，1)表示在1，2維度互換
        q = self.text(q)  # [batch, max_len, 1280]

        if self.visual_normalization:  # 规范化处理
            v = v / (v.norm(p=2, dim=2, keepdim=True) + 1e-12).expand_as(v)  # [batch, max_obj, 2048]
        v_mask = v_mask.float()
        q_mask = q_mask.float()
        
        v, q = self.interIntraBlocks(v, q, v_mask, q_mask)  # 模态间和模态内的一个注意流 SingleBlock

        answer = self.classifier(v, q, v_mask, q_mask)  # 经过DAFA注意后，分类得到答案
...
class SingleBlock(nn.Module):
    ...
    def forward(self, v, q, v_mask, q_mask):
        """
        v: visual feature      [batch, num_obj, feat_size] 最开始是2400，后来是512
        q: question            [batch, max_len, feat_size] 最开始是1280，后来是512
        v_mask                 [batch, num_obj]
        q_mask                 [batch, max_len]
        """
        # transfor features
        v = self.v_lin(self.drop(v))  # [batch, num_obj, 512]
        q = self.q_lin(self.drop(q))  # [batch, max_len, 512]

        # 用一个循环将多个块叠加，v,q特征不断更新
        for i in range(self.num_block):
            v, q = self.interBlock(v, q, v_mask, q_mask)  # 模态间的块
            v, q = self.intraBlock(v, q, v_mask, q_mask)  # 模态内的动态的块
...
class MultiBlock(nn.Module):
    ...
    def __init__(self, num_block, v_size, q_size, output_size, num_head, drop=0.0):
        ...
        for i in range(num_block - 1):
            ...
            blocks.append(DyIntraModalityUpdate(output_size, output_size, output_size, num_head, drop))
        self.multi_blocks = nn.ModuleList(blocks)  # 保存列表中的子模块。
...
class InterModalityUpdate(nn.Module):
    """
    Inter-modality Attention Flow 模态间的注意流
    """
    def __init__(self, v_size, q_size, output_size, num_head, drop=0.0):
        ...
        self.v_lin = nn.Linear(v_size, output_size * 3)  # output_size * 3 = 512 * 3
    ...
    def forward(self, v, q, v_mask, q_mask):
        ...
        # transfor features
        v_trans = self.v_lin(self.drop(self.relu(v))) # 512->300
        q_trans = self.q_lin(self.drop(self.relu(q)))

        # mask all padding object/word features
        v_trans = v_trans * v_mask.unsqueeze(2)  # [batch, num_obj, 300]*[batch, num_obj, 1]
        q_trans = q_trans * q_mask.unsqueeze(2)

        # split for different use of purpose
        # 切片，100块，每块大小为3,
        v_k, v_q, v_v = torch.split(v_trans, v_trans.size(2) // 3, dim=2) # v_trans.size(2) // 3 = outsize = 100
        q_k, q_q, q_v = torch.split(q_trans, q_trans.size(2) // 3, dim=2)

        # apply multi-head  公式3-5
        # 在2维切块，切完之后每一块大小为8
        ...
        # 通过一个全连接输出 公式10-11
        updated_v = self.v_output(self.drop(cat_v))  # [batch, num_obj, 512]
        updated_q = self.q_output(self.drop(cat_q))
        return updated_v, updated_q

class DyIntraModalityUpdate(nn.Module):
    """
    Dynamic Intra-modality Attention Flow  动态的模态内注意流
    """
    def __init__(self, v_size, q_size, output_size, num_head, drop=0.0):
        ...
        self.q_output = nn.Linear(output_size, output_size)  # output_size = 512
    ...
    def forward(self, v, q, v_mask, q_mask):
        ...
        v_mean = (v * v_mask.unsqueeze(2)).sum(1) / v_mask.sum(1).unsqueeze(1)  # [batch, feat_size]
        q_mean = (q * q_mask.unsqueeze(2)).sum(1) / q_mask.sum(1).unsqueeze(1)  # [batch, feat_size]

        # 产生用于其他模态的通道式调节门（一个模态自注意时，受到另一个的模态的影响）  公式14-15
        ...
        # 算出这个概率（门）后，将其与原模态中的Q和K进行点积，得到的就是增加了另一模态约束之后的Q和V
        # 公式16
